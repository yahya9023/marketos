...
export default function CategoriesPage() {
  ...
  return <main className="min-h-screen bg-[#0c1b2a] p-4 text-slate-950 sm:p-8"><section className="mx-auto max-w-3xl rounded-2xl bg-[#f5f7f2] p-5 sm:p-7"><h1 className="text-3xl font-black text-[#0c1b2a]">Categories</h1>{error && <p className="mt-4 rounded-lg bg-rose-50 p-3 text-sm font-bold text-rose-700">{error}</p>}{success && <p className="mt-4 rounded-lg bg-[#e4eee7] p-3 text-sm font-bold text-[#346154]">{success}</p>}<form onSubmit={submit} className="mt-5 flex flex-wrap gap-2"><input required value={name} onChange={(event) => setName(event.target.value)} placeholder="Category name" className="h-11 min-w-0 flex-1 rounded-lg border border-[#d6e0d9] bg-white px-3 font-semibold" /><button className="rounded-lg bg-[#0c1b2a] px-4 py-2 text-sm font-black text-white">{editing ? 'Save' : 'Add category'}</button></form><div className="mt-6 space-y-2">{categories.map((category) => <div key={category.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[#dce5df] bg-white p-3"><span className="font-bold">{category.name}</span><div className="flex gap-2"><span className="text-xs font-bold text-slate-500">{category.active ? 'Active' : 'Inactive'}</span><button type="button" onClick={() => { setEditing(category); setName(category.name); }} className="text-xs font-bold text-[#346154]">Edit</button><button type="button" onClick={() => void toggle(category)} className="text-xs font-bold text-rose-600">{category.active ? 'Deactivate' : 'Activate'}</button></div></div>)}</div></section></main>;
}